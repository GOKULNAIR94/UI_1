...
app.controller('mainCont', function($scope, $http, $location) {
    ...
    $scope.update = function(id, parentdata, childdata, close) {
        ...
        $http(req).then(function(result) {
            if (childdata.length > 0) {
                $scope.updateChild(id, childdata, 0, close);
            }
        }).then(function(response) {
    ...
    $scope.updateChild = function (id, childdata, x, close) {
        $http({
            method: 'POST',
            ...
            if ( x < childdata.length -1 )
                $scope.updateChild(id, childdata, x + 1, close);
            else {
                if (close == 0) {
                    alert('Saved!');
                    $scope.deal($scope.currentRecord);
